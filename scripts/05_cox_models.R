# Fit Cox proportional hazards models for the propensity-score-matched cohort.
#
# Models are fit for two outcomes, four covariate sets, and two cohorts
# (overall matched and age 65+ matched). Validation runs before each model;
# failed validations are recorded and skipped rather than halting the pipeline.
#
...
if (!file.exists(matched_path)) {
  stop(
    "Matched data file not found: ", matched_path, "\n",
    "Run scripts/04_propensity_matching.R first."
  )
}
...
df_matched <- readRDS(matched_path)

cat("Matched data loaded.\n")
cat("  Rows:    ", nrow(df_matched), "\n")
cat("  Columns: ", ncol(df_matched), "\n\n")
...
df_age65 <- create_age_subcohort(df_matched, age_cutoff = 65)

cohort_list <- list(
  list(label = "Overall matched cohort", data = df_matched),
  list(label = "Age 65+ matched cohort", data = df_age65)
)
...
if (length(all_results) > 0L) {
  ...
  stop(
    ...
    "Check validation errors above and re-run scripts/04_propensity_matching.R if needed."
  )
}
